...
public class MainFrame implements Observer {
	...
	private void initComponents() {
		...
		mainFrame.setTitle("JMineSweeper By NCH");
	...
	public void update(Observable o, Object arg) {
		...
		if(notifingObject.getKey().equals("start")){
			...
			GameVO overGame = (GameVO) notifingObject.getValue();
			//System.out.println(overGame.getLevel());
			//System.out.println(overGame.getGameResultStae());
			OperationQueue.isRunning=false;
			TimeLable.gameRun=false;
			

			
			if(overGame.getGameResultStae().equals(GameResultState.FAIL)){
				body.setBorder(new javax.swing.border.TitledBorder(
						new javax.swing.border.TitledBorder(""), "You fail!",
						javax.swing.border.TitledBorder.CENTER,
						javax.swing.border.TitledBorder.DEFAULT_POSITION));
				MyStaticData.reFlashData();
				
				startButton.setIcon(Images.GAMELOSE);
			}
			if(overGame.getGameResultStae().equals(GameResultState.SUCCESS)){
				body.setBorder(new javax.swing.border.TitledBorder(
						new javax.swing.border.TitledBorder(""), "You win!",
						javax.swing.border.TitledBorder.CENTER,
						javax.swing.border.TitledBorder.DEFAULT_POSITION));
				
				startButton.setIcon(Images.GAMEWIN);
				
				if(MyStaticData.isRun==0){
					MyStaticData.winTotal[0]++;
					if(MyStaticData.winTime[0].equals("Unknow")){
						MyStaticData.winTime[0]=overGame.getTime()+"";
					}else{
						if(overGame.getTime()<Integer.parseInt(MyStaticData.winTime[0])){
							MyStaticData.winTime[0]=overGame.getTime()+"";
						}
					}
					MyStaticData.reFlashData();
				}
				if(MyStaticData.isRun==1){
					MyStaticData.winTotal[1]++;
					if(MyStaticData.winTime[1].equals("Unknow")){
						MyStaticData.winTime[1]=overGame.getTime()+"";
					}else{
						if(overGame.getTime()<Integer.parseInt(MyStaticData.winTime[1])){
							MyStaticData.winTime[1]=overGame.getTime()+"";
						}
					}
					MyStaticData.reFlashData();
				}
				if(MyStaticData.isRun==2){
					MyStaticData.winTotal[2]++;
					if(MyStaticData.winTime[2].equals("Unknow")){
						MyStaticData.winTime[2]=overGame.getTime()+"";
					}else{
						if(overGame.getTime()<Integer.parseInt(MyStaticData.winTime[2])){
							MyStaticData.winTime[2]=overGame.getTime()+"";
						}
					}
					MyStaticData.reFlashData();
				}
				if(MyStaticData.isRun==3){
					MyStaticData.winTotal[3]++;
					if(MyStaticData.winTime[3].equals("Unknow")){
						MyStaticData.winTime[3]=overGame.getTime()+"";
					}else{
						if(overGame.getTime()<Integer.parseInt(MyStaticData.winTime[3])){
							MyStaticData.winTime[3]=overGame.getTime()+"";
						}
					}
					MyStaticData.reFlashData();
		...
		for (int j = 0; j <IOHelper.LINE; j++) {
			System.out.println(MyStaticData.gameTotal[j]);
		}
	}
	...
	private void restart(int mineBoardHeight,int mineBoardWidth,String type) {
		...
		try {
			...
			case "大":
				...
				break;
			case "自定义":
				MyStaticData.gameTotal[3]++;
				MyStaticData.isRun=3;
				break;
			}
		} catch (Exception e) {
			// TODO 自动生成的 catch 块
		...
		Thread timeMachine=new Thread(time);
		timeMachine.start();
